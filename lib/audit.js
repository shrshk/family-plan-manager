...
const { transformAuditResultsToUpdates } = require('./transform')
...
exports.runAudit = async (auditStartDate, auditEndDate) => {
    ...
    try {
        ...
        const auditUpdates = transformAuditResultsToUpdates(dfAuditFinal.values, AUDIT_SHEET_NAME, auditHeader)
...
const filterNamedRanges = (namedRangeNames) => {
    return namedRangeNames.filter((namedRangeName) => {
        ...
        const date = new moment({ year, month: monthNum-1, day })
