...
exports.transformTransactionsToUpdates = function(transactions) {
  ...
  const updates = transactions.map(function(transaction, i) {
    return {
      range: `${process.env.SHEETS_TRANSACTIONS_SHEET_NAME}!A${i + 2}:E${i + 2}`,
      values: [Object.values(transaction)]
    }
  });

  updates.push({
    range: `${process.env.SHEETS_TRANSACTIONS_SHEET_NAME}!A1:E1`,
...
exports.transformAuditResultsToUpdates = (auditResults, auditSheetName, auditHeader) => {
  let auditResultValues = auditResults.map(result => Object.values((result)))
  return [auditHeader, ['SearchKey', 'AmountOwed', 'AmountPaid'], ...auditResultValues]
}